...
/*************************************************************************************/
/*OUTPUT VERTEX PIXEL COORDINATES FOR BOTH MAP AND CROPPED MAP                       */
/*This is just a list of the vertices that appear on the path in the order of the    */
/*first time the vertex was encounter (but not duplicate for subsequent times).      */
/*************************************************************************************/
void OutputVertexPixelCoordinates(int vertex) {
int x = VX[vertex];
int y = VY[vertex];
if (Vcheck[vertex]) return;
Vcheck[vertex] = 1;
fprintf(vfile,"%d %d\n",
   (int)( x * ((double)MapHeightPixels / (double)MapHeightFeet) ),
   (int)( y * ((double)MapWidthPixels  / (double)MapWidthFeet ) )
   );
fprintf(vcfile,"%d %d\n",
...
/*Output coordinates of edge ends (will do nothing if a vertex has previously been output).*/
OutputVertexPixelCoordinates(Estart[edge]);
OutputVertexPixelCoordinates(Eend[edge]);
